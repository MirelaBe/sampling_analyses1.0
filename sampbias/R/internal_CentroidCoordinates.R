.CentroidCoordinates <- function(x, testdist = 0.1, limits = NULL, 
                                 testtype = c("country","provinces", "both"), 
                                 referencedat = NULL) {
  dat <- x
  if (is.null(referencedat)) {
    testdat <- sampbias::centroids
    
    if (testtype[1] == "country") {
      testdat <- testdat[testdat$type == "country", ]
    }
    if (testtype[1] == "province") {
      testdat <- testdat[testdat$type == "province", ]
    }
  } else {
